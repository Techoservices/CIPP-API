...
.DESCRIPTION
    ...
    1. Fetches all User AND Shared Mailboxes.
    2. Checks/Updates the Exception rule FIRST (Ensures it has higher priority).
    3. Flushes old "Display Name" rules.
    4. Recreates rules in batches (shards) of 200 names (Placed at bottom of priority list).
...
"@
        ...
        # ----------------------------------------------------------------
        # SECTION 1: Manage Exception Rule (Must run BEFORE Spoofing Rules)
        # ----------------------------------------------------------------
        ...
        $ExceptParams = @{
            ...
            # No Priority Set: It will go to bottom if new, but Spoofing rules will go BELOW it.
        }
        ...
        for ($i = 0; $i -lt $TotalNames; $i += $BatchSize) {
             ...
             $RuleParams = @{
                ...
                # Rules added here are appended to the bottom of the list, 
                # ensuring they are processed AFTER the Exception Rule.
        ...
        $summary = "Completed in $formattedTime. Names Protected (User+Shared): $($results.UsersFound). Rules Created: $($results.RulesCreated). Exceptions Updated: $($results.ExceptionRuleUpdated)."
