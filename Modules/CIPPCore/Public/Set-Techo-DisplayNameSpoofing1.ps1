...
function Set-Techo-DisplayNameSpoofing {
    ...
    try {
        ...
        # HTML Disclaimer - THE CLOSING TAG MUST BE AT THE START OF THE LINE
        $RuleHtml = @"
...
"@
        ...
        # ----------------------------------------------------------------
        # We need separate params for Set vs New because Set requires Identity
        $ExceptParams = @{
        ...
        try {
            if ($ExistingExceptionRule) {
                ...
                # Clone params and add Identity for the Set command
                ...
                Write-LogMessage -headers $Headers -API $APIName -tenant $TenantFilter -message "Exception rule not found. Creating..." -Sev Debug
                
                # Clone params and add Enabled for the New command
                $NewParams = $ExceptParams.Clone()
                $NewParams.Add("Enabled", $true)
