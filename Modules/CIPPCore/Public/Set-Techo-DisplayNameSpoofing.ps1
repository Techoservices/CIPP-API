...
.DESCRIPTION
    ...
    1. Fetches all User Mailboxes.
    2. Flushes old "Display Name" rules to prevent duplication/drift.
    3. Recreates rules in batches (shards) of 200 names.
    4. Checks/Updates the Exception rule.
...
"@
        ...
        # 1. Fetch User Mailboxes
        Write-LogMessage -headers $Headers -API $APIName -tenant $TenantFilter -message "Fetching User Mailboxes..." -Sev Info
        ...
        if (-not $DisplayNames) {
            Write-LogMessage -headers $Headers -API $APIName -tenant $TenantFilter -message "No user mailboxes found. Aborting." -Sev Info
            return "No mailboxes found."
        }
        
        $results.UsersFound = $DisplayNames.Count
        Write-LogMessage -headers $Headers -API $APIName -tenant $TenantFilter -message "Found $($results.UsersFound) users." -Sev Info


        # 2. Flush Old Rules
        # We need to find rules matching our prefix
        try {
            $AllRules = New-ExoRequest -tenantid $TenantFilter -cmdlet 'Get-TransportRule' -ErrorAction SilentlyContinue
            ...
            Write-LogMessage -headers $Headers -API $APIName -tenant $TenantFilter -message "Error checking/removing old rules. Continuing." -Sev Warning
        }

        # 3. Create Batched Rules
        $BatchSize = 200
        $Counter = 1
        ...
        for ($i = 0; $i -lt $TotalNames; $i += $BatchSize) {
             ...
             # Force array type to ensure New-TransportRule accepts it as a list
        ...
        # 4. Manage Exception Rule
        Write-LogMessage -headers $Headers -API $APIName -tenant $TenantFilter -message "Checking Exception Rule..." -Sev Info
        ...
        # Check if exception rule exists by looking at our previously fetched $AllRules or fetching fresh
        try {
            $ExceptionRule = New-ExoRequest -tenantid $TenantFilter -cmdlet 'Get-TransportRule' -cmdParams @{ Identity = $ExceptionRuleName } -ErrorAction SilentlyContinue
            
            if ($ExceptionRule) {
                Write-LogMessage -headers $Headers -API $APIName -tenant $TenantFilter -message "Exception rule exists. Updating..." -Sev Debug
                New-ExoRequest -tenantid $TenantFilter -cmdlet 'Set-TransportRule' -cmdParams $ExceptParams | Out-Null
            } else {
                Write-LogMessage -headers $Headers -API $APIName -tenant $TenantFilter -message "Exception rule missing. Creating..." -Sev Debug
                # Add Enabled param only for creation
            ...
            $results.ExceptionRuleUpdated = $true
        }
        catch {
             Write-LogMessage -headers $Headers -API $APIName -tenant $TenantFilter -message "Error managing Exception rule." -Sev Warning
        }

        # Generate summary
        $totalTime = (Get-Date) - $startTime
        $formattedTime = "{0:mm}m {0:ss}s" -f $totalTime
        $summary = "Completed in $formattedTime. Users: $($results.UsersFound). Rules Created: $($results.RulesCreated). Exceptions Updated: $($results.ExceptionRuleUpdated)."
